...
function App() {
  ...
  useEffect(() => {
    ...
    for (const lot of lots) {
      ...
      if (!(lot.id in lotOrders) && (lot.lot_status === 'sold' || lot.lot_status === 'accepted')) {
        loadLotOrder(lot.id)
      }
    }
  ...
  async function loadLots() {
    ...
    try {
      ...
      setLotsError(requestError.message)
    } finally {
  ...
  function scrollToLots() {
    document.getElementById('lots')?.scrollIntoView({ behavior: 'smooth', block: 'start' })
  }
  ...
  async function acceptBuyerOffer(lotId, offerId) {
    ...
    try {
      ...
      setLots((current) => current.map((item) => (item.id === lotId ? updatedLot : item)))
    } catch (requestError) {
      setBuyerOfferViews((current) => ({
        ...current,
  ...
  const lotStatusLabel = (value) => ({ draft: t('status.draft'), active: t('status.available'), matched: t('status.matched'), offered: t('status.offered'), accepted: t('status.accepted'), sold: t('status.sold'), rejected: t('status.rejected'), cancelled: t('status.cancelled') }[value] || t('status.unknown'))
  const trendLabel = (value) => ({ RISING: t('trend.rising'), FALLING: t('trend.falling'), STABLE: t('trend.stable'), INSUFFICIENT_DATA: t('trend.insufficient') }[value] || (value ? value.replaceAll('_', ' ') : t('trend.insufficient')))
  const buyerTypeLabel = (value) => ({ trader: t('buyerType.trader'), processor: t('buyerType.processor'), retailer: t('buyerType.retailer'), aggregator: t('buyerType.aggregator'), exporter: t('buyerType.exporter'), institution: t('buyerType.institution') }[value] || value)
  const offerStatusLabel = (value) => ({ pending: t('offerStatus.pending'), accepted: t('offerStatus.accepted'), rejected: t('offerStatus.rejected'), expired: t('offerStatus.expired'), countered: t('offerStatus.countered') }[value] || value)
  const orderStatusLabel = (value) => ({ created: t('orderStatus.created'), confirmed: t('orderStatus.confirmed'), packed: t('orderStatus.packed'), in_transit: t('orderStatus.in_transit'), delivered: t('orderStatus.delivered'), cancelled: t('orderStatus.cancelled') }[value] || value)
  ...
  return <main className="app-shell">
    ...
    <ConversationalAssistant crops={crops} locations={locations} language={language} t={t} onCreated={handleAssistantCreated} onSeeRecommendation={scrollToLots} />
